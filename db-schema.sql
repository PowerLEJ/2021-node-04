...
CREATE TABLE IF NOT EXISTS `boards` (
  ...
  PRIMARY KEY (`id`),
  KEY `UserId` (`UserId`),
  CONSTRAINT `boards_ibfk_1` FOREIGN KEY (`UserId`) REFERENCES `seq-users` (`id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8;

-- 내보낼 데이터가 선택되어 있지 않습니다.

-- 테이블 leepower.books 구조 내보내기
...
-- 테이블 leepower.seq-users 구조 내보내기
...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `createdAt` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `api` enum('KA','NA','FA','GO','LO') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT 'LO',
